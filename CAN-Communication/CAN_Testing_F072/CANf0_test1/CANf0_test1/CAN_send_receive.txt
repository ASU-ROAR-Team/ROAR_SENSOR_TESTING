...
/* USER CODE BEGIN PTD */
#define MASTERCAN 1
#define SLAVECAN !MASTERCAN
volatile uint8_t messageReceived = 0;
char statusMsg[32] = "Initializing...";
...
HAL_StatusTypeDef can_sender(uint8_t data[], CAN_TxHeaderTypeDef msg, uint32_t mb);
/* USER CODE END PFP */
...
void can_irq(CAN_HandleTypeDef *pcan) {
  ...
  if (HAL_CAN_GetRxMessage(pcan, CAN_RX_FIFO0, &rxHeader, rxData) == HAL_OK) {
    if (rxHeader.StdId == 0x103 && rxHeader.DLC == 6) {
      if (strncmp((char*)rxData, "Hello!", 6) == 0) {
        messageReceived = 1;
        if (HAL_GetTick() - lastStatusUpdate > 300) {
          strncpy(statusMsg, "RECEIVED: OK", sizeof(statusMsg)-1);
...
HAL_StatusTypeDef can_sender(uint8_t data[], CAN_TxHeaderTypeDef msg, uint32_t mb) {
  ...
  if (HAL_CAN_GetTxMailboxesFreeLevel(&hcan) > 0) {
    status = HAL_CAN_AddTxMessage(&hcan, &msg, data, &mb);
    if (HAL_GetTick() - lastStatusUpdate > 300) {
      strncpy(statusMsg, (status == HAL_OK) ? "SENT: OK" : "SEND FAILED", sizeof(statusMsg)-1);
      lastStatusUpdate = HAL_GetTick();
    }
  } else {
    if (HAL_GetTick() - lastStatusUpdate > 300) {
      strncpy(statusMsg, "BUSY: Try again", sizeof(statusMsg)-1);
      lastStatusUpdate = HAL_GetTick();
    }
  }
  statusMsg[sizeof(statusMsg)-1] = '\0';
